...
export async function execute(ctx) {
    ...
    const modelDiscoveryTimeoutSec = asNumber(config.modelDiscoveryTimeoutSec, 90);
    if (preflightModelCheck && !executionTargetIsRemote) {
        await ensurePiModelConfiguredAndAvailable({
            model,
            command,
            cwd,
            env: runtimeEnv,
            timeoutSec: modelDiscoveryTimeoutSec,
        });
    }
    const extraArgs = (() => {
