...
import { asNumber, asString, parseObject, ensurePathInEnv,
  asStringArray, } from "@paperclipai/adapter-utils/server-utils";
import {
...
export async function testEnvironment(
  ...
  // In this fork, model discovery is OPT-IN via preflightModelCheck (default: false).
  const preflightModelCheck = config.preflightModelCheck === true;
  if (preflightModelCheck && !targetIsRemote && canRunProbe) {
    try {
  ...
  } else if (preflightModelCheck && canRunProbe) {
    // Verify model is in the list (only when preflight is opted-in)
    try {
      const discovered = await discoverPiModelsCached({ command, cwd, env: runtimeEnv });
