...
const DEFAULT_MODEL_DISCOVERY_TIMEOUT_SEC = 90;
const DEFAULT_MODEL_DISCOVERY_GRACE_SEC = 10;
...
export async function discoverPiModels(input: {
  ...
  const timeoutSec = typeof input.timeoutSec === "number" && input.timeoutSec > 0
    ? input.timeoutSec
    : DEFAULT_MODEL_DISCOVERY_TIMEOUT_SEC;
  ...
  const result = await runChildProcess(
    ...
    ["--list-models"],
    {
      ...
      env: runtimeEnv,
      timeoutSec,
      graceSec: DEFAULT_MODEL_DISCOVERY_GRACE_SEC,
      onLog: async () => {},
    },
  );
...
export async function discoverPiModelsCached(input: {
  ...
  const models = await discoverPiModels({ command, cwd, env, timeoutSec: input.timeoutSec });
